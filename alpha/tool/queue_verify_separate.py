...
def search_par_set(one_path):
    ...
    label_head2.config(text='0 bad sets')
    label_head3.config(text='0 complete sets')
...
def button_open2_clicked():
    ...
    if len(indices) == 1:
        ...
        one_path = folder_path + "\\" + base_name + ".par2"
        if os.path.isfile(one_path) == False: # If index file doesn't exist, search another file.
            one_path = glob.glob(folder_path + "/" + base_name + ".vol*.par2")[0]
